...
impl Robot {
    pub fn new(name: RobotName, x: usize, y: usize) -> Robot {
        Robot {
            name,
            pos: Coord { x, y },
        }
        ...
        for x in 0..height {
            ...
            match direction {
                ...
                CardinalDirection::Down => {
                    map[y][x] = Coord { x, y: height - 1 };
                }
                CardinalDirection::Left => {
                    map[y][x] = Coord { x: 0, y };
                }
                CardinalDirection::Right => {
                    map[y][x] = Coord { x: width - 1, y };
                }
            };
            ...
            match direction {
                CardinalDirection::Up => {
                    for wall in walls {
                        if wall.direction == Direction::Vertical
                            && wall.coord.x == x
                            && wall.coord.y <= y
                        {
                            map[y][x] = Coord {
                                x,
                                y: map[y][x].y.max(wall.coord.y),
                            };
                        }
                    }
                }
                CardinalDirection::Down => {
                    for wall in walls {
                            ...
                            && wall.coord.y > y
                        {
                            map[y][x] = Coord {
                                x,
                                y: map[y][x].y.min(wall.coord.y - 1),
                            };
                        }
                    }
                }
                ...
                CardinalDirection::Right => {
                    for wall in walls {
                        ...
                        {
                            map[y][x] = Coord {
                                x: map[y][x].x.min(wall.coord.x - 1),
                                y,
                            };
                        }
                    }
...
impl Grid {
    pub fn new(width: usize, height: usize, walls: Vec<Wall>) -> Grid {
        ...
        for direction in &[
            ...
            move_maps.insert(
                direction.clone(),
                build_move_lookup_map(width, height, &walls, direction),
            );
        }
        Grid {
            width,
...
impl Position {
        ...
        match direction {
            CardinalDirection::Down => {
                for robot in &self.robots {
                    if robot.pos.x == unconstrained_end.x
                        && start.y < robot.pos.y
                        && robot.pos.y <= unconstrained_end.y
                    {
            ...
            CardinalDirection::Right => {
                for robot in &self.robots {
                    if robot.pos.y == unconstrained_end.y
                        && start.x < robot.pos.x
    ...
    pub fn make_move(&self, robot_move: &Move) -> Position {
        let mut robots: Vec<Robot> = self
            ...
            .clone()
            .into_iter()
            .filter(|r| r.name != robot_move.name)
            .collect();
        robots.push(Robot {
